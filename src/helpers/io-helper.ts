...
export class IoHelper {
    public static async ask(question: string, flag: string): Promise<string> {
    ...
    public static async askYesNo(question: string, flag: string): Promise<boolean> {
    ...
    public static async askSelect<T>(question: string, options: { value: T, label: string }[], flag: string): Promise<T> {
        let index: number;
        if (this.hasOption(flag)) {
